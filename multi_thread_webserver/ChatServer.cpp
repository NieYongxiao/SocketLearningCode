...
void ChatServer::chatServerFunc(int clnt_sock){
    ...
    if(str_len == 0 ){
        ...
        analysis_message(buffer,clnt_sock);
    }
}

void ChatServer::analysis_message(char *buffer,int clnt_sock){
    ...
    if(strcmp(method, "GET") == 0) handle_get(string(pathname),clnt_sock);
    else{
        //chatServerErrorHandler("method is not get");
        //此时已经连接了clnt_sock，并且此线程不会进行后续操作，所以需要关闭
        chatServerErrorHandler("strcmp error");
    }
}  //Aborted 

void ChatServer::handle_get(string&& path,int clnt_sock){
    if(path == "/"){  //此时输入路径只有“/”
        path="/home/xiaosa/web_server_source_material/default.html";
    }
    else if(path.find("/home/xiaosa/web_server_source_material/" )!= string::npos){
        if(path.back() == '/') chatServerErrorHandler("path is not a valid path");
    }
...
void ChatServer::send_html_file(char* buf,int clnt_sock,FILE* file_reader){
    unique_lock<mutex> lock(mtx);
...
void ChatServer::send_txt_file(char* buf,int clnt_sock,FILE* file_reader){
    unique_lock<mutex> lock(mtx);
    sprintf(buf, "HTTP/1.1 200 OK\r\n");
...
void ChatServer::send_jpg_file(char* buf,int clnt_sock,FILE* file_reader) {
    unique_lock<mutex> lock(mtx);
    sprintf(buf, "HTTP/1.1 200 OK\r\n");
...
void ChatServer::send_gif_file(char* buf,int clnt_sock,FILE* file_reader) {
    unique_lock<mutex> lock(mtx);
    sprintf(buf, "HTTP/1.1 200 OK\r\n");
...
void ChatServer::chatServerErrorHandler(const string&& str){
    unique_lock<mutex> lock(mtx);
    cout<<str<<endl;
    lock.unlock();
    exit(1);
